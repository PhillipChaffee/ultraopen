import { createChild, promptChild, type SpawnOptions, type SpawnOutcome } from "./spawn.js"
...
export async function spawnStructured(client: OpencodeClient, options: SpawnOptions): Promise<StructuredResult> {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    const outcome = await promptChild(client, sessionID, prompt, options)
...
function requiredSummary(schema: Record<string, unknown>): string[] {
  ...
  return required
    ...
    .map((key) => {
      const sub = props[key]
      const type = typeof sub === "object" && sub !== null ? (sub as Record<string, unknown>)["type"] : undefined
      const describedAs = typeof type === "string" ? type : "value"
