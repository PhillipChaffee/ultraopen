...
import { newBootId, reapOrphans } from "../src/server/resume/reaper.js"
...
describe("reapOrphans", () => {
  ...
  test("aborts every child of a run abandoned by a dead process", async () => {
    ...
    const result = await reapOrphans(client, "current-boot", { env })
    expect(aborted).toEqual(["child-1", "child-2"])
    expect(result).toEqual({ runs: 1, sessions: 2, failures: 0 })
  })

  test("marks the run orphaned so a later start does not sweep it again", async () => {
    await seed()
    const { client } = makeClient()
    await reapOrphans(client, "current-boot", { env })

    expect((await readManifest("wf_dead001", env))?.status).toBe("orphaned")

    const second = makeClient()
    expect(await reapOrphans(second.client, "current-boot", { env })).toEqual({ runs: 0, sessions: 0, failures: 0 })
  ...
  test("leaves runs from the CURRENT boot alone", async () => {
    ...
    expect(await reapOrphans(client, "current-boot", { env })).toEqual({ runs: 0, sessions: 0, failures: 0 })
  ...
  test("counts aborts that fail and still marks the run orphaned", async () => {
    ...
    const result = await reapOrphans(client, "current-boot", { env })
    expect(result).toEqual({ runs: 1, sessions: 1, failures: 1 })
    expect((await readManifest("wf_dead001", env))?.status).toBe("orphaned")
  })
  ...
  test("reports what it released", async () => {
    ...
    const notes: string[] = []
    const { client } = makeClient()
    await reapOrphans(client, "current-boot", { env, onNote: (note) => notes.push(note) })
  ...
  test("stays silent and cheap when there is nothing to reap", async () => {
    const notes: string[] = []
    const { client } = makeClient()
    expect(await reapOrphans(client, "boot", { env, onNote: (note) => notes.push(note) })).toEqual({
      runs: 0,
      sessions: 0,
      failures: 0,
    })
  ...
  test("a run with no recorded children is still marked orphaned", async () => {
    ...
    const { client } = makeClient()
    expect(await reapOrphans(client, "current-boot", { env })).toEqual({ runs: 1, sessions: 0, failures: 0 })
  })
})
...
describe("failure tolerance", () => {
  test("an unreadable data root reaps nothing instead of throwing at startup", async () => {
    ...
    ).toEqual({ runs: 0, sessions: 0, failures: 0 })
  })
...
describe("non-throwing contract", () => {
  test("never rejects, so callers can fire-and-forget without a handler", async () => {
    ...
    await expect(reapOrphans(client, "current-boot", { env })).resolves.toBeDefined()
  })
})
