...
function makeClient(
  ...
  const create = options.create ?? ((): Promise<CreateResult> => Promise.resolve({ data: { id: "child-1" } }))
  const prompt =
    options.prompt ??
...
describe("interpret", () => {
  test("no schema + text parts -> ok:true with the LAST text part's text", () => {
    ...
    const result = interpret(response, "session-1", false)
  ...
  test("schema + info.structured set -> ok:true and structured passed through", () => {
    ...
    const response: PromptResponse = { info: baseInfo({ structured }), parts: [textPart("ignored")] }
    const result = interpret(response, "session-1", true)
    if (!result.ok) throw new Error("expected success")
...
describe("spawn", () => {
  ...
  test("the session id is registered after a successful spawn", async () => {
    ...
    const result = await spawn(client, { prompt: "p", runId: "run-1", parentSessionID: "parent-1", label: "worker" })
    if (!result.ok) throw new Error("expected success")
